...
def setup_training_options(
    ...
    # Transfer learning.
    resume     = None, # Load previous network: 'noresume' (default), 'ffhq256', 'ffhq512', 'ffhq1024', 'celebahq256', 'lsundog256', 'latest', <file>, <url>
    ...
    if p is not None:
        ...
        if aug != 'fixed' and resume != 'latest':
            raise UserError('--p can only be specified with --aug=fixed or --resume=latest')
        if not 0 <= p <= 1:
...
#----------------------------------------------------------------------------
  ...
  # TODO: Add descriptions for other configs
...
transfer learning source networks (--resume):
  ...
  latest         Find last network snapshot saved in outdir
...
def main():
    ...
    group.add_argument('--cfg',   help='Base config (default: auto)', choices=['auto', 'auto_no_ramp', '11gb-gpu','11gb-gpu-complex', '24gb-gpu','24gb-gpu-complex', '24gb-2gpu-complex' '48gb-gpu', 'stylegan2', 'paper256', 'paper512', 'paper1024', 'cifar', 'cifarbaseline', 'aydao'])
